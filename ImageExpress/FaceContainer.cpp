...
void FaceContainer::closeOutputFile(){
	std::cout << std::endl;
	std::cout << "========================================================" << std::endl;
	std::cout << "Output File : " << m_cFile.GetFilePath() << std::endl;
	m_cFile.Close();
...
void FaceContainer::printAllResult(){
	...
	//print progress of output
	std::cout << std::endl;
	std::cout << "Now Analyse Faces and Print Result:" << std::endl;
...
//"FaceboxX, FaceboxY, FaceboxW, FaceboxH, "
void FaceContainer::printFaceSize(const EMOTIENT::FacetSDK::Face& face, bool nonBlank){ 
	if (!nonBlank) {
		printBlankComma(4);
		return;
	}
	...
	buff.append (std::to_string((long double)faceLocation.x) + ", " 
	           + std::to_string((long double)faceLocation.y) + ", "
